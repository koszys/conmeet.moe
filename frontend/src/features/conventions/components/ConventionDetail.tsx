...
function DetailSkeleton() {
  return (
    <main className="flex-1">
      <div className="border-ink border-b-2 bg-zinc-100 dark:bg-zinc-900" />
      <div className="mx-auto max-w-6xl animate-pulse px-4 py-10 md:px-6">
  ...
  return (
    <main className="flex-1">
      ...
      {!isLoading && !isError && convention && (
        <>
          <div className="border-ink relative h-56 overflow-hidden border-b-2 bg-accent-soft/40 md:h-80 dark:bg-zinc-900">
            {convention.banner ? (
              <img src={convention.banner} alt="" className="h-full w-full object-cover" />
            ) : (
              <MikuSilhouette className="text-accent/30 absolute -right-4 bottom-0 hidden h-64 w-auto md:block" />
            )}
          </div>
          ...
          <div className="mx-auto max-w-6xl px-4 py-10 md:px-6">
            ...
            <div className="mt-6 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
              <div>
                <h1 className="font-display text-3xl tracking-wide [text-shadow:2px_2px_0_color-mix(in_srgb,var(--ink)_22%,transparent)] sm:text-5xl">
                  {convention.name}
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  {convention.map_url && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="border-ink text-ink hover:border-accent-pop hover:text-accent-pop inline-flex items-center gap-2 border-2 px-5 py-2.5 text-xs font-bold tracking-widest bg-white uppercase transition-colors dark:bg-zinc-900 dark:text-zinc-100"
                    >
            ...
            {convention.description && (
              <section className="border-ink mt-10 border-b-2 border-t-2 border-dashed py-10">
                <div
